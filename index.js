...
function addGamesToPage(games) {
   ...
   for (const game of games){
        ...
        newGameCard.innerHTML = `<img class = "game-img" src="${game.img}">
        <h2>${game.name}</h2> 
...
const numGames = GAMES_JSON.reduce((total, game) => {
    return total += 1;
}, 0)
...
const numUnfundedGames = unfundedGames.reduce((total, game) => {
    return total += 1;
}, 0)
console.log(numUnfundedGames)

// create a string that explains the number of unfunded games using the ternary operator
let displayStr = `A total of $${totalAmountRaised.toLocaleString('en-US')} has been raised for ${numGames} games. Currently, ${numUnfundedGames} ${numUnfundedGames === 1 ? "games" : "game"} remains unfunded. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
const newDomElement = `Backers: ${displayStr}`;
descriptionContainer.append(newDomElement);
